...
paso8_estado_hrs.py
===================
PASO 8: Experimento con el memristor en Alta Resistencia (HRS).

Se mantiene el dispositivo cerca de HRS (x ~ 0, R_M ~ R_off).
Objetivo: Demostrar que una resistencia alta dificulta la activacion neuronal
          (se reduce la corriente transmitida y baja el numero de spikes).
"""
...
# Configuramos el memristor para que se mantenga en HRS
p_mem = StrukovParameters(
    R_on   = 100.0,
    R_off  = 500_000.0, # Aumentado pedagogicamente para bloquear totalmente
    D      = 10e-9,
    # mu_v extremadamente bajo para que x casi no cambie y se quede en HRS
    mu_v   = 1e-16,
    w_init = 0.01,  # Inicia en x=0.01 -> R_M ~ 495 kOhm
...
ax1.text(0.01, 0.86, f"Estado: Mantenido en HRS\n$R_M \\approx$ {R_mem[0]/1e3:.1f} kOhm",
...
style_ax(ax2, C_IMEM, r"$I_{mem}$ (µA)", r"② $I_{mem}(t)$ — Corriente reducida por la alta resistencia")
...
style_ax(ax3, C_VC, r"$V_c$ (V)", r"③ $V_c(t)$ — Carga lenta debido a la baja corriente", ylim=[-0.05, 1.25])
...
style_ax(ax4, C_VOUT, r"$V_{out}$ (V)", r"④ $V_{out}(t)$ — Menor cantidad de spikes", ylim=[-0.05, 0.80])
...
fig.suptitle(
    "PASO 8 — Experimento Memristor en HRS (Alta Resistencia)\n"
    f"Spikes bloqueados: {n_spikes} (vs 3 en simulacion de aprendizaje)",
    fontsize=13, fontweight="bold", y=0.975
)

out_dir  = os.path.join(os.path.dirname(__file__), os.path.join(os.path.dirname(__file__), "..", "..", "output_modular"))
os.makedirs(out_dir, exist_ok=True)
out_path = os.path.join(out_dir, "paso8_estado_hrs.png")
