"""
Prueba 2x2_06: Programación LTD selectiva.
"""
...
def draw_2x2_06(gui, n_pulses=40, V_pulse=-1.0, target='M22', **kwargs):
    """
    Programa solo una celda con LTD y muestra el efecto.

    Nota de Física Memristiva (Strukov):
    La asimetría en |ΔG| entre LTP (iniciado en x0 ≈ 0.10) y LTD (iniciado en x0 ≈ 0.80)
    se debe a la respuesta dependiente del estado del memristor Strukov.
    """
    ...
    for ii in range(2):
        for jj in range(2):
            cb.set_conductance(ii, jj, 300e-6)

    G_initial = cb.G_matrix.copy()

    syn = MemristiveSynapse(cb.memristors[i, j])
    ltd = LTDRule(n_pulses=n_pulses, V_pulse=V_pulse)
    ...
    ax1.plot(range(len(G_history)), G_history * 1e6,
             '-s', ms=4, lw=2, color='#f38ba8', label=f'G_{target}')
    ax1.axhline(G_initial[i, j] * 1e6, color='gray',
                ls='--', lw=1, label='G inicial (x0≈0.80)')
    ax1.set_xlabel('Número de pulsos', color='white')
    ax1.set_ylabel('G (μS)', color='white')
    ax1.set_title(f'LTD en {target} ({n_pulses} pulsos desde x0 alto)',
    ...
    return {
        ...
        'metrics': {
            ...
            'Asimetría Strukov': 'LTD más rápido en x0 alto (física real)',
        }
    }
