...
def test_07_ltp():
    ...
    G_history = ltp.apply(syn, dt=1e-3)

    G_final = G_history[-1]
    delta_G = (G_final - G_inicial) * 1e6

    assert G_final > G_inicial, "G debe subir con LTP"
    ...
    ax.plot(range(len(G_history)), G_history * 1e6, 'b-o', ms=4, lw=1.5)
    ...
    ax.set_title(f'Prueba 7: LTP (+1V, 40 pulsos) — ΔG = {delta_G:+.4f} μS')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    fig_path = os.path.join(out_dir, 'fig_07_ltp.png')
    plt.savefig(fig_path, dpi=300)
    plt.close()

    print("=" * 60)
    print("PRUEBA 7: PROGRAMACIÓN LTP")
    print("=" * 60)
