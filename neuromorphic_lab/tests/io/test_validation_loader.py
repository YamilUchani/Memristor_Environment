...
def test_prezioso_validation_data_loader():
    loader = ValidationDataLoader()
    data = loader.load_all(dataset="prezioso")

    assert data is not None, "El cargador de datos de Prezioso devolvió None."
    required_keys = [
        "t_i", "i_val_mA", "i_val", "t_v", "v_val", "v_interp",
        "r_val_kohm", "g_val_us"
    ]
    for key in required_keys:
        assert key in data, f"Falta la clave '{key}' en los datos de Prezioso."
        assert len(data[key]) > 0, f"El vector '{key}' está vacío."
        assert isinstance(data[key], np.ndarray), f"El elemento '{key}' debe ser un ndarray."

    assert not np.isnan(data["i_val"]).any(), "Corriente I de Prezioso contiene NaN."
    assert not np.isnan(data["v_val"]).any(), "Voltaje V de Prezioso contiene NaN."
